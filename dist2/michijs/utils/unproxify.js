...
export function unproxify(val) {
  ...
  if (item && typeof item === "object") {
    if (Array.isArray(item)) return cloneArray(item, unproxify);
    if (item instanceof Date) return cloneDate(item);
    if (item instanceof Map) return cloneMap(item, unproxify);
    if (item instanceof Set) return cloneSet(item, unproxify);
