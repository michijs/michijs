...
export function useObserve(item, initialObservers) {
  if (item) {
    ...
    if (typeofItem === "object") {
      if (Array.isArray(item)) return observeArray(item, initialObservers);
      ...
      if (item instanceof Date) return observeDate(item, initialObservers);
      if (item instanceof Map) return observeMap(item, initialObservers);
      if (item instanceof Set) return observeSet(item, initialObservers);
