 ...
 */
export const customObjectSet = (initialObservers) => (target, property, newValue, receiver) => {
    ...
    if (target.$value) {
        ...
        if (oldValue) {
            ...
            if (typeof oldValue.$value === "function")
 ...
 */
export const customObjectApply = (proxy, initialObservers) => (target, _, args) => {
    ...
    if (args.length > 0) {
        ...
        if (target.$value && valueType === "object")
            ...
            proxy().$value = useObserve(newValue, initialObservers).$value;
        return;
    }
...
export function observeCommonObject(item, initialObservers = []) {
    ...
    const newObservable = new ProxiedValue(item && Object.getPrototypeOf(item) === Object.prototype
        ? cloneCommonObject(item, (value) => useObserve(value, newInitialObservers))
        : item, initialObservers);
