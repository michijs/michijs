...
import { formatToKebabCase, addStylesheetsToDocumentOrShadowRoot, } from "../utils";
...
export function createCustomElement(tag, elementOptions) {
    const { events, attributes, reflectedAttributes, lifecycle, render, adoptedStyleSheets, extends: extendsObject, shadow = extendsObject ? false : { mode: "open" }, computedStyleSheet, cssVariables, reflectedCssVariables, methods, formAssociated = false, } = elementOptions ?? {};
    ...
    const mappedAdoptedStyleSheets = adoptedStyleSheets
        ? Object.values(adoptedStyleSheets).map((x) => typeof x === "function" ? x(internalCssSelector) : x)
        : undefined;
    ...
    class MichiCustomElementResult extends classToExtend {
        ...
        addInitialStyleSheets(selector, target) {
            ...
            if (computedStyleSheet) {
                this.$michi.styles.computedStyleSheet ??= useStyleSheet(computedStyleSheet.bind(this)(selector));
                addStylesheetsToDocumentOrShadowRoot(target, this.$michi.styles.computedStyleSheet);
            }
            if (mappedAdoptedStyleSheets)
                addStylesheetsToDocumentOrShadowRoot(target, ...mappedAdoptedStyleSheets);
        }
        constructor() {
            super();
        ...
        connectedCallback() {
            if (!this.$michi.shadowRoot &&
                ...
                this.addInitialStyleSheets(`.${this.$michi.styles.className}`, this.getRootNode());
            }
            this.connected?.();
