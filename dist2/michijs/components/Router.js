 ...
 * @returns {Node}
 */
export const Router = ({ as: asTag, routes, parentRoute, enableCache, ...attrs }, options) => {
    const el = asTag
        ? create({
            jsxTag: asTag,
            attrs,
    ...
    const matchedRoute = useComputedObserve(() => {
        return Object.keys(routes).find((key) => HistoryManager.matches(urlFn(key, parentRoute)().pathname, true));
    }, [HistoryManager]);
    ...
    bindObservable(matchedRoute, (newMatchedRoute) => {
        ...
        if (newMatchedRoute) {
            if (cache[newMatchedRoute])
                el.replaceChildren(cache[newMatchedRoute]);
            else {
