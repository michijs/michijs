 ...
 */
export const ElementInternals = createFunctionalComponent(({ children, errorMessage, formValue, tabIndex = 0, validityStateFlags = { customError: true }, ...aria }, options) => {
    ...
    if (self && isMichiCustomElement(self) && self.$michi.internals) {
        if (errorMessage) {
            ...
            bindObservableToRef(errorObservable, self, (newValue, self) => {
                ...
                // @ts-ignore
                unproxify(validityStateFlags), unproxify(newValue.errorMessage));
            });
        }
        if (formValue)
        ...
        Object.entries({ tabIndex, ...aria }).forEach(([key, value]) => {
            if (self.$michi.internals)
                if (key in self.$michi.internals)
                    bindObservableToRef(value, self, (newValue, self) => (self.$michi.internals[key] = newValue));
                else if (key in self)
                ...
                // Some browsers still dont support internals
                    ...
                    bindObservableToRef(value, self, (newValue, self) => setAttribute(self, formattedKey, newValue));
                }
        });
    }
    return children;
