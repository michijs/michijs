...
- **`Symbol.dispose` on `ReactiveValue`** — Reactive values now implement the [Explicit Resource Management](https://github.com/tc39/proposal-explicit-resource-management) protocol, allowing them to be used with the `using` declaration. Disposal clears all subscribers and releases the held value reference. The method is idempotent and safe to call multiple times. The `[Symbol.dispose](): void` signature is also part of the `ReactiveValuePort` contract.
