...
- **Dependency auto-tracking** for `useComputedObserve` and `useAsyncComputedObserve` — When `deps` is omitted, dependencies are automatically detected by tracking observable reads during callback execution. Uses diff-based subscription management to minimize overhead.
