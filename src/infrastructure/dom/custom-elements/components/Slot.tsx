...
import { isElement } from "../../rendering/typewards/isElement";
import { getShadowRoot } from "../getShadowRoot";
import type { WithChildren } from "../../../platform/types/WithChildren";
import type { GetJSXProps } from "../../../platform/types/GetJSXProps";
  ...
  lifecycle: {
    didMount() {
      ...
      const callback = (nodeList: NodeList) => {
        ...
        for (const x of nodeList as unknown as Node[]) {
          if (
 ...
 */
export const Slot = (attrs: WithChildren<HTMLElements["slot"]>, factory) =>
  getShadowRoot(factory.contextElement) ? (
    <slot {...attrs} />
  ) : (
    <MichiSlot
      {...(attrs as GetJSXProps<typeof MichiSlot>)}
      _={{
