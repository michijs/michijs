...
const getCookies = () => document.cookie.split(';').reduce((previousValue, x) => {
...
if (window.cookieStore) {
  // @ts-ignore
  const cookieStoreChange = new ObservableFromEventListener(cookieStore, "change");
  ...
  cookieStoreChange.subscribe((e) => {
    if (document.cookie !== currentCookies) {
      ...
      // @ts-ignore
      cookieStoreObservable.notify([...e.changed.map(x => x.name), ...e.deleted.map(x => x.name)]);
    }
  });
}
...
export class CookieStorage implements Storage {
  ...
  constructor(setOptions?: CookieStorageConstructor) {
    this.setOptions = Object.entries(setOptions ?? {}).map(([key, value]) => `;${formatToKebabCase(key)}=${value}`).join('')
  }
  clear(): void {
