...
export function observe<T>(
  ...
  if (item && typeof item === "object") {
    ...
    // Built-in methods access them directly, not via [[Get]]/[[Set]] internal methods. So Proxy can’t intercept that.
      ...
      return observeDate(item, initialObservers) as unknown as Observable<T>;
    else if (item instanceof Map)
      return observeMap(item, initialObservers) as unknown as Observable<T>;
    else if (item instanceof Set)
      return observeSet(item, initialObservers) as unknown as Observable<T>;
    else if (Object.getPrototypeOf(item) === Object.prototype)
      return observeCommonObject(item, initialObservers) as Observable<T>;
    // console.error(`The object with path "${props.propertyPath}" cannot be observed ${item}`)
  }
  return observeValue(item, initialObservers) as unknown as Observable<T>;
}
