  ...
  implements ProxyHandler<ProxiedValueV2<T>>
{
  ...
  deleteProperty(target: ProxiedValueV2<T>, property) {
    return (
      target.handler.deleteProperty?.(target, property) ??
  ...
  apply(target: ProxiedValueV2<T>, _, args) {
    return (
      target.handler.apply?.(target, _, args) ?? Reflect.apply(target, _, args)
    );
  }
  ...
  getOwnPropertyDescriptor(target: ProxiedValueV2<T>, property) {
    return (
      target.handler.getOwnPropertyDescriptor?.(target, property) ??
  ...
  has(target: ProxiedValueV2<T>, property) {
    return (
      target.handler.has?.(target, property) ?? Reflect.has(target, property)
    );
  }

  get(target: ProxiedValueV2<T>, p, receiver) {
    return (
      target.handler.get?.(target, p, receiver) ??
