...
export class ArrayProxyHandler<T extends Array<any>>
  extends ObjectProxyHandler<T>
  implements ObservableProxyHandler<ProxiedValueV2<T>, Array<any>>
{
  $newItemsCallback =
    (target, bindedTargetProperty) =>
    (...args: T[]) => {
      const proxiedArray = this.getInitialValue(target, args);
    ...
    splice:
      ...
      (start, deleteCount, ...items) => {
        const result = bindedTargetProperty(
          start,
          deleteCount,
          ...items.map((x) => this.createProxyChild(target, x)),
        );
        return result;
      },
  };
  ...
  getInitialValue(target, unproxifiedValue: Array<any>): T {
