...
export const Link = createFunctionalComponent<LinkProps>(({ url, ...attrs }, options) => {
  const href = useComputedObserve(() => {
    const unproxifiedUrl = unproxify(url);
    return typeof unproxifiedUrl === "object" && "href" in unproxifiedUrl ? unproxifiedUrl.href : unproxifiedUrl
  }, [url])
  return jsx("a", {
    ...attrs,
    href,
