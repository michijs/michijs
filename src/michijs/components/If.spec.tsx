...
function jsIfTests(enableCache: boolean) {
  ...
  const nodeReturn = create(If(observable, [
    [true, () => "True"],
    [undefined, () => "Undefined"]
  ], () => "False", {
    as: 'div',
...
describe("If", () => {
  describe("When the condition is a css variable", () => {
    ...
    const cssIfReturn = If(cssVariables.rotation, [
      ["180deg", "red"],
      ["360deg", "green"],
    ], "blue")

    it("should return the expected string", () => {
      expect(cssIfReturn).toBe(`if(style(--rotation:180deg):red;style(--rotation:360deg):green;else:blue)`);
    });
  })
