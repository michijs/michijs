...
export const AsyncComponent = <P, const T = CreateFCResult>(
  { as: asTag, promise, loadingComponent, catch: errorComponent, then, ...attrs }: AsyncComponentProps<P, T>,
  ...
  const renderCallback = (p: PromiseType<P>) => (typeof p === "function" ? p() : p).then(res => render(res)).catch((e) => {
    if (errorComponent){
