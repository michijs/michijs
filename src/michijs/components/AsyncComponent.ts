...
type AsyncComponentProps<T> = ExtendableComponentWithoutChildren<T> & {
  promise: Promise<PromiseReturnType> | (() => Promise<PromiseReturnType>) | (() => Promise<{ default: PromiseReturnType }>);
...
export const AsyncComponent = <const T = FC>(
  ...
  const render = (promiseResult: PromiseReturnType | { default: PromiseReturnType }) => {
    const oldEl = el;
    const Res: PromiseReturnType = promiseResult && typeof promiseResult === 'object' && 'default' in promiseResult ? promiseResult.default : promiseResult;

    el = create(Res && typeof Res === 'function' ? jsx(Res) : Res, options) as ChildNode & ParentNode;
