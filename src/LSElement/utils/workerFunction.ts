...
export function workerFunction<T extends (...args: any) => any>(functionToExecute: T) {
  return (...args: Parameters<T>) => new Promise<ReturnType<T>>((resolve, reject) => {
    ...
    worker.postMessage({ functionToExecute: functionToExecute.toString(), args });
    return worker;
  });
}