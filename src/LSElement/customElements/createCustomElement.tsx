...
>(el: LSElementConfig<EX, EL>, elementProperties: LSElementProperties<M, T, E, S, A, RA, FRA> & ThisType<Self<M, T, E, A, RA, EL>>): CreateCustomElementResult<A, FRA, RA, M, T, E, EL> {
  ...
  if (elementProperties.events)
    Object.entries(elementProperties.events).forEach(([key, value]) => value.init(key));
  ...
  class LSCustomElementResult extends (classToExtend as CustomElementConstructor) implements LSCustomElement {
        ls: LSCustomElement['ls'] = {
          store: lsStore.apply(this, [{ state: { ...elementProperties.attributes, ...elementProperties.reflectedAttributes }, transactions: elementProperties.transactions }]) as ReturnType<typeof lsStore>,
          alreadyRendered: false,
          adoptedStyleSheets: [],
          pendingTasks: 0,
          rerenderCallback: (propertyThatChanged: Set<PropertyKey> | PropertyKey) => {
            if (elementProperties.observe)
              Object.entries<() => void>(elementProperties.observe).forEach(([key, observer]) => {
        ...
        willMount = elementProperties.lifecycle?.willMount;
        didMount = elementProperties.lifecycle?.didMount;
        didUnmount = elementProperties.lifecycle?.didUnmount;
        willUpdate = elementProperties.lifecycle?.willUpdate;
        didUpdate = elementProperties.lifecycle?.didUpdate;
        willReceiveAttribute = elementProperties.lifecycle?.willReceiveAttribute;
        render = elementProperties.render as LSCustomElement['render'];
        ...
        constructor() {
          ...
          if (elementProperties.methods)
            Object.entries(elementProperties.methods).forEach(([key, value]) => defineMethod(this, key, value));
          ...
          if (elementProperties.events)
            Object.entries(elementProperties.events).forEach(([key, value]) => defineEvent(this, key, value));
          if (elementProperties.reflectedAttributes)
            Object.keys(elementProperties.reflectedAttributes).forEach(key => {
          ...
          if (elementProperties.subscribeTo)
            Object.entries(elementProperties.subscribeTo).forEach(([key, value]) => {
        ...
        static get observedAttributes() { return elementProperties.reflectedAttributes ? Object.keys(elementProperties.reflectedAttributes).map(key => formatToKebabCase(key)) : []; }
